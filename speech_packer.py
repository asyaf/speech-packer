...
class Packer(object):
    ...
    def __init__(self):
        self._to_pack = set()
        self._packed = set()
        self._commands = set([self.Command.ADDITION, self.Command.PACKING, self.Command.DELETE,
                              self.Command.LIST, self.Command.STOP])

    @staticmethod
    def _get_item():
        print('What is the name of the item?')
        return speech_analyzer.process_single_phrase()

    def _add_item(self):
        text = Packer._get_item()
